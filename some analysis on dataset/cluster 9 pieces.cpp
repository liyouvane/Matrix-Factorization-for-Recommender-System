...
/*void cluster(){
 ...
 for (int i=1;i<=M;i++){
 int dis1=distance(T[i],T[ker_u[1]],N);
 int dis2=distance(T[i],T[ker_u[2]],N);
 if (dis1>=dis2) type_u[i]=1;
 else type_u[i]=2;
 }
 for (int j=1;j<=N;j++){
 int dis3=distance(T1[j],T1[ker_i[1]],M);
 int dis4=distance(T1[j],T1[ker_i[2]],M);
 if (dis3>=dis4) type_i[j]=1;
 else type_i[j]=2;
 }
